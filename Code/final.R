...
test_index <- sample(nrow(all_data), 0.3*nrow(all_data)) 
...
#Test Model
res1 <- predict(model1, data_test, type = 'class') #class type return factor ('0', '1')
head(res1)
...
model_cv <- train(death~., data = data_train, trControl = train_control, method = 'rpart')
model_cv$resample
